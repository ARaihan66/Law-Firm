        ...
        <div className="mx-auto w-full max-w-7xl px-8 pt-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <div className=" ml-3 mr-3">
              <img src={Logo} alt="Logo" className="logo mx-auto my-auto" />
              <p className="text-justify">
                Mustafizur Rahman & Associates is one of the Top law firm in
                Dhaka, Bangladesh offering full service with specialization in
                corporate, commercial and investment legal practice. This Law
                Firm consists of experienced lawyers and consultants, providing
                one-stop solutions to the clients.
              </p>
            </div>
            {SITEMAP?.map(({ title, item, links }) => (
              <ul key={title}>
                <Typography
                  variant="small"
                  className="mb-3 ml-8 font-bold  text-navyblue text-[24px]"
                >
                  {title}
                </Typography>
                {links?.map((l, i) => (
                ...
                {item?.map((link) => (
                  <li key={link}>
                    <div className="py-1.5 ml-8 font-medium transition-colors text-navyblue">
                      {link}
                    </div>
                  </li>
                ))}
